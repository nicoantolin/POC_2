var initonmenuordentrabajoserviciotec = function(){
	...
	$('#sTecnico').find('#guardarSTecnico').click(function(){
		if($('#sTecnico').valid()){
			var ot = {
				...
				id : ordenTrabajo.id,		
				excluyeCCalidad : $('#sTecnico').find('#ot\\.excluyeCCalidad').is(':checked')
			};
			SSTFacade.saveSTecnicoOT(ot,{async:true,callback:function(){
				$.alerts.okButton = '&nbsp;Ok&nbsp;';
				jAlert('La informacion se ha guardado correctamente','Información');
			}});
		}
	});
...
var loadonmenuordentrabajoserviciotec = function(){
	SSTFacade.getOTById(ordenTrabajo.id,{async:false,callback:function(ot){
		SSTFacade.getServicioTecnicoByOT(ordenTrabajo.id,{async:false,callback:function(stecnico){
			if(stecnico != null){
				$('#noEnviadoST').hide();
				$('#enviadoST').show();
				stecnico.ot.contratoEmitido = !stecnico.ot.contratoEmitido;
				stecnico.ot.excluyeCCalidad = ot.excluyeCCalidad;
				$('#sTecnico').loadObject(stecnico);
				$('#sTecnico').find('#ot\\.numeroContrato').attr('readonly', !ordenTrabajo.contratoEmitido);
			} else {
				$('#noEnviadoST').show();
