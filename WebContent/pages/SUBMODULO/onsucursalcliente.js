var initonsucursalcliente = function() {
	...
	SSTFacade.getModuloByNombreUsuario('onsucursalterminar',{async:false, callback:function(modulo) {
        ...
        if(ordenTrabajo.tipoDocumento=="factura")
		{
         document.getElementById('giroFactura').style.display="block";
        }
	else
		{
		document.getElementById('giroFactura').style.display="none";
		}
    }});
	...
	$('#cliente').find('#guardarCliente').click(function(){
		if(validaClienteForm()){
			var giroEmpresa= document.getElementById('giro').value;
			var cliente = {};
			cliente = $('#cliente').serializeObject();
			ordenTrabajo.numeroFolio = $('#cliente').find('#numeroFolio').val();
            ordenTrabajo.cliente = cliente;
			SSTFacade.terminarOrdenTrabajo(cliente,ordenTrabajo,giroEmpresa,{async:true,callback:function(oTResultado){
	...
	var guardarCliente = function(){
		if(validaClienteForm()){
			var cliente = {};
			var giroEmpresa= document.getElementById('giro');
			cliente = $('#cliente').serializeObject();
            ordenTrabajo.numeroFolio = $('#cliente').find('#numeroFolio').val();
            debugger;
			ordenTrabajo.cliente = cliente; 
			SSTFacade.terminarOrdenTrabajo(cliente,ordenTrabajo,giroEmpresa,{async:false,callback:function(oTResultado){
				ordenTrabajo = oTResultado;
				if( ordenTrabajo.estado.id == 10003000 || ordenTrabajo.estado.id == 10008000){
					if(ordenTrabajo.tipo.codigo == 'CA'){
...
var initonsucursalcliente = function() {
	...
	$('#cliente').find('#guardarCliente').click(function(){
		if(validaClienteForm()){
			...
			ordenTrabajo.numeroFolio = $('#cliente').find('#numeroFolio').val();
            ordenTrabajo.cliente = cliente;
            ordenTrabajo.giroEmpresa=giroEmpresa;
            debugger; 
			SSTFacade.terminarOrdenTrabajo(cliente,ordenTrabajo,giroEmpresa,{async:true,callback:function(oTResultado){
				ordenTrabajo = oTResultado;
				if( ordenTrabajo.estado.id == 10003000 || ordenTrabajo.estado.id == 10008000){
...
var loadonsucursalcliente = function(ordenTrabajo) {
	...
	document.getElementById('giro').value=ordenTrabajo.giroEmpresa;
};
